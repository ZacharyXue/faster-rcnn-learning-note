...
# nms--> non-maximum suppression, faster rcnn中引入
from torchvision.ops import nms

from torch import nn
# 该preprocess只是对于图片进行合适的缩放
...
def nograd(f):
    """
    该函数将传入的函数置于with t.no_grad()之下
    从而使得传入函数内的tensor将不参与训练
    """
    def new_f(*args, **kwargs):
        # with 后面的语句语句被求值后，返回对象的 __enter__() 被调用，
        # 这个方法的返回值将被赋值给as后面的变量;
        # with 后代码块全部执行后调用返回对象的 __exit__()
        with t.no_grad():
...
class FasterRCNN(nn.Module):
    ...
    def _suppress(self, raw_cls_bbox, raw_prob):
        ...
        # 这里使用 concatenate() 而不是 stack() ！！！
        # 以 bbox 为例，bbox append的是经过掩码操作的 cls_bbox_l，
        # cls_bbox_l第一个维度是该类别所有的 proposal region，
        # 所以应该使用 concatenate 在所有类别 cls_bbox_l
